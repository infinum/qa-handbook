...
Comparative testing is allways a good thing to do, it could illuminate a lot of issues quickly, and will help to compare implementations on Android and iOS directly.
...
4. Cash on delivery is rather hard to test. You can test in tandem with the client when a user chooses this option. Cash on delivery does not go through a service, rather the payment is collected when a user recives the ordered item.
...
9. Verify that you recive a receipt for the purchase, otherwise the payment should not go through.
...
11. If there is tax calculation within the payment process, check how it is calculated and if its applied well.

Most important thing here is to follow the client requests, because the client is going to end up with that payment process and is going to have to work with it. Think of the users :)
